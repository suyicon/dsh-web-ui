...
import { batFileName, desktopFileName, renderBatWrapper, renderDesktopEntry, renderLauncherScript, renderShortcutInstaller, scriptFileName, type LauncherPlatform, type LauncherSpec } from './core/launcher.ts'
...
export async function createDesktopShortcut(deps: LauncherRoutesDeps): Promise<CreateResult> {
  ...
  if (platform === 'win32') {
    // Write the BAT wrapper so the .lnk targets cmd.exe + launcher.bat
    // instead of powershell.exe directly. This avoids the Windows Defender
    // HEUR:Trojan/LNK.Agent.b heuristic which detects .lnk files pointing to
    // powershell.exe with suspicious flags.
    const batPath = join(scriptsDir, batFileName())
    await writeFile(batPath, renderBatWrapper(launcherPath))
    const installerPath = join(scriptsDir, 'install-shortcut.ps1')
    await writeFile(installerPath, renderShortcutInstaller({ batPath, desktopPath: iconPath, homeDir: home, iconLocation: iconIco ?? 'cmd.exe,0' }))
