...
export async function createDesktopShortcut(deps: LauncherRoutesDeps): Promise<CreateResult> {
  ...
  if (platform === 'win32') {
    // Write the VBS wrapper so the .lnk targets wscript.exe + launcher.vbs
    // instead of powershell.exe with suspicious flags. This avoids the Windows
    // Defender HEUR:Trojan/LNK.Agent.b heuristic.
