...
/** File name of the BAT wrapper (Windows only, keeps .lnk off powershell.exe). */
export function batFileName(): string {
  return 'launcher.bat'
}

/**
 * Render the BAT wrapper that calls the PowerShell launcher. The .lnk targets
 * cmd.exe + this .bat instead of powershell.exe directly, avoiding the Windows
 * Defender LNK heuristic HEUR:Trojan/LNK.Agent.b which detects .lnk files
 * pointing to powershell.exe with suspicious flags.
 * @param ps1Path - absolute path of the launcher.ps1 script.
 * @returns the BAT script body.
 */
export function renderBatWrapper(ps1Path: string): string {
  return [
    '@echo off',
    'rem DeepSeek Harness launcher wrapper (generated by dsh-desktop-launcher)',
    // No -WindowStyle Hidden here: launcher.ps1 hides its own console via the
    // Win32 ShowWindow API, keeping this flag out of the bat content.
...
function renderPowerShell(spec: LauncherSpec): string {
  return [
    ...
    '',
    // Hide the console window early so the bat wrapper does not need the
    // suspicious -WindowStyle Hidden flag (Windows Defender may flag it).
    "Add-Type -Namespace Win32 -Name Native -MemberDefinition '[System.Runtime.InteropServices.DllImport(\"kernel32.dll\")] public static extern IntPtr GetConsoleWindow(); [System.Runtime.InteropServices.DllImport(\"user32.dll\")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow);' -ErrorAction SilentlyContinue",
    '$consoleHwnd = [Win32.Native]::GetConsoleWindow()',
    'if ($consoleHwnd -ne [IntPtr]::Zero) { [void][Win32.Native]::ShowWindow($consoleHwnd, 0) }',
    '',
 ...
 * the Desktop .lnk pointing at the BAT wrapper (launcher.bat), avoiding
 * Windows Defender heuristic on direct powershell.exe references. The BAT
 * wrapper is written alongside the launcher.ps1 by the route handler.
 * @param opts - the BAT path, icon, and working-directory paths.
 * @returns the installer body.
 */
export function renderShortcutInstaller(opts: {
  /** Absolute path of launcher.bat (the .lnk target). */
  batPath: string
  ...
  const { batPath, desktopPath, homeDir, iconLocation } = opts
  return [
    ...
    `$shortcut.TargetPath = ${psSingle('cmd.exe')}`,
    `$shortcut.Arguments = ${psSingle(`/c "${batPath}"`)}`,
