...
/** File name of the VBS wrapper (Windows only, shields .lnk from Defender heuristic). */
export function vbsFileName(): string {
  return 'launcher.vbs'
}

/**
 * Render the VBS wrapper that hides the powershell.exe invocation from the
 * .lnk target. The shortcut targets wscript.exe + this VBS instead of
 * powershell.exe with suspicious flags, avoiding Windows Defender heuristic
 * HEUR:Trojan/LNK.Agent.b.
...
export function renderVbsWrapper(ps1Path: string): string {
  // VBScript doubles quotes inside string literals.
  const escaped = ps1Path.replaceAll('"', '""')
  return [
    "' DeepSeek Harness launcher wrapper (generated by dsh-desktop-launcher)",
    "' VBS wrapper to avoid Windows Defender LNK heuristic on powershell.exe arguments.",
 ...
 * the Desktop .lnk pointing at the VBS wrapper (launcher.vbs), avoiding
 * Windows Defender heuristic on direct powershell.exe references. The VBS
 * wrapper is written alongside the launcher.ps1 by the route handler.
 * @param opts - the VBS path, icon, and working-directory paths.
 * @returns the installer body.
 */
export function renderShortcutInstaller(opts: {
  /** Absolute path of launcher.vbs (the .lnk target). */
  vbsPath: string
  ...
  const { vbsPath, desktopPath, homeDir, iconLocation } = opts
  return [
    ...
    `$shortcut.TargetPath = ${psSingle('wscript.exe')}`,
    `$shortcut.Arguments = ${psSingle(vbsPath)}`,
